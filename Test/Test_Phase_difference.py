...
phi_fad_range = range(0, 420, 60)
phi_fad_range = np.array(phi_fad_range) * np.pi / 180

results = {}

total_steps = len(phi_fad_range) * computer_num
progress_bar = tqdm(total=total_steps, desc='Processing phase difference values', unit='steps')

for phi_fad in phi_fad_range:
    Error_Model = []
    Error_DeepCNN_Model = []
    Error_CV_Model = []
    Error_Music = []
    ...
    print(f"phi_fad = {phi_fad}")
    print("Proposed_Model RMSE:", results[phi_fad]['Model'])
    print("DeepCNN RMSE:", results[phi_fad]['DeepCNN'])
    print("CV RMSE:", results[phi_fad]['CV'])
    print("Music RMSE:", results[phi_fad]['Music'])
    print("CBF RMSE:", results[phi_fad]['CBF'])
    print("MVDR RMSE:", results[phi_fad]['MVDR'])
    print("PM RMSE:", results[phi_fad]['PM'])
    print("SS_Music RMSE:", results[phi_fad]['SS_Music'])
    print("Toeplitz_Music RMSE:", results[phi_fad]['Toeplitz_Music'])
    print()
...
df.to_csv('Phi_fad0to360_SNR10_Snap200_-34.76_46.59.csv')
